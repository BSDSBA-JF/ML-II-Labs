...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("RANDOM FOREST WITH CENTRALITY FEATURES")
    print("Genshin Impact Pull Prediction")
    print("="*60 + "\n")
    
    # Train single model with alpha=0.2 (best performing from report)
    print("Training model with alpha=0.2 (optimal value from report)...\n")
    result = train_random_forest_with_centrality(alpha=0.2, verbose=True)
    
    print("\n" + "="*60)
    print("TRAINING COMPLETE")
    print("="*60)
    print(f"\nFinal Model: RandomForestRegressor with {result['best_params']['n_estimators']} estimators")
    print(f"Test Set R²: {result['metrics']['R2']:.4f}")
    print(f"Test Set MAE: {result['metrics']['MAE']:,.2f}")
    print(f"Test Set RMSE: {result['metrics']['RMSE']:,.2f}")
    
    # Optional: Uncomment to compare different alpha values
    # print("\n" + "="*60)
    # print("COMPARING DIFFERENT ALPHA VALUES")
    # print("="*60)
    # alpha_comparison = compare_alphas(alphas=[0.2, 0.3, 0.4], verbose=False)
    
    # Optional: Uncomment to plot feature importances
    # print("\nGenerating feature importance plot...")
    # plot_feature_importances(result, top_n=20)
